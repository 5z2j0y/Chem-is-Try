...
import time
import logging
from draw_hand import draw_landmarks  # 导入新的绘制函数
        ...
        if not hand_result_queue.empty():
            ...
            if hand_result.hand_landmarks:
                for hand_landmarks in hand_result.hand_landmarks:
                    landmark_points = [(int(lm.x * frame.shape[1]), int(lm.y * frame.shape[0])) for lm in hand_landmarks]
                    frame = draw_landmarks(frame, landmark_points)
